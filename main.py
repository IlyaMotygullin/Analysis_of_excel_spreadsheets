...
def create_diaposon(excel_obj: pd.ExcelFile, sheet: str, database_for_date: dict, list_date_user: list) -> None:
    ...
    get_date = datetime.strptime(list_date_user[0], '%Y-%m-%d %H:%M:%S')
    get_date1 = datetime.strptime(list_date_user[1], '%Y-%m-%d %H:%M:%S')
...
def convert_to_target_format(date_value):
...
def save_formatted_excel_with_backup(excel_obj: pd.ExcelFile, sheet_name: str, dict_date_coordinate: dict, date_mapping: dict, output_path: str = None, backup=True):
    df = excel_obj.parse(sheet_name=sheet_name, header=None)
    reverse_mapping = {v: k for k, v in date_mapping.items()}

    for original_key, (row_idx, col_idx) in dict_date_coordinate.items():
        if original_key in date_mapping:
            df.iat[row_idx, col_idx] = reverse_mapping[original_key]

    if output_path is None:
        desktop = os.path.join(os.path.expanduser('~'), 'Desktop')
        output_path = os.path.join(desktop, f"formatted{sheet_name}.xlsx")
...
for key in dict_date_coordinate.keys():
    if isinstance(key, (datetime, pd.Timestamp)):
        ...
        key_str = str(key)
        match = re.search(r'\b\d{2}\.\d{2}\.\d{4}\b', key_str)

        if match:
            found_date = match.group(0)
            try:

                dt = datetime.strptime(found_date, '%d.%m.%Y')
...
try:
    saved_file = save_formatted_excel_with_backup(
        excel_obj=excel_file,
        sheet_name=user_opinion,
        dict_date_coordinate=dict_date_coordinate,
        date_mapping=date_mapping,
        backup=True  # Создать бэкап перед изменением
    )
    print(f"✓ Файл успешно сохранен: {saved_file}")
...
# Ищем введенные даты
found1 = date_mapping.get(write_date_info_1)
found2 = date_mapping.get(write_date_info_2)

if isinstance(found1, (datetime, date)) and isinstance(found2, (datetime, date)):
    if found1 is not None and found2 is not None:
        # Преобразуем даты в формат для create_diaposon
        date1_str = convert_to_target_format(found1)
        date2_str = convert_to_target_format(found2)

        list_for_create_diaposon = [date1_str, date2_str]

        # Выводим информацию для отладки
        print(f"\nНайдены даты:")
        print(f"Первая дата (оригинал): {found1}, преобразована в: {date1_str}")
        print(f"Вторая дата (оригинал): {found2}, преобразована в: {date2_str}")
        ...
        print('Вы ошиблись при вводе даты, перепроверьте информацию.')
        if found1 is None:
            print(f"Дата '{write_date_info_1}' не найдена в списке.")
        if found2 is None:
            print(f"Дата '{write_date_info_2}' не найдена в списке.")
else:
    # Поиск дат в таблице
    df = excel_file.parse(sheet_name=user_opinion, header=None)
    ...
    for row_idx in range(df.shape[0]):
        for col_idx in range(df.shape[1]):
            ...
            if isinstance(cell_value, (datetime, date)):
                ...
                if date_str == write_date_info_1 and not date1_coord:
                    date1_coord = (row_idx, col_idx)
                if date_str == write_date_info_2 and not date2_coord:
                    date2_coord = (row_idx, col_idx)
            elif isinstance(cell_value, str) and search_date(cell_value):
                match = re.search(r'\b\d{2}\.\d{2}\.\d{4}\b', cell_value)
                if match and match.group() == write_date_info_1 and not date1_coord:
                    date1_coord = (row_idx, col_idx)
                if match and match.group() == write_date_info_2 and not date2_coord:
                    date2_coord = (row_idx, col_idx)

    # Если нашли обе даты
    if date1_coord and date2_coord:
        # Добавляем найденные даты в словарь координат
        try:
            dt1 = datetime.strptime(write_date_info_1, '%d.%m.%Y')
            dt2 = datetime.strptime(write_date_info_2, '%d.%m.%Y')

            dict_date_coordinate[dt1] = date1_coord
            dict_date_coordinate[dt2] = date2_coord

            # Формируем список дат для функции create_diaposon
            date1_str = convert_to_target_format(dt1)
            date2_str = convert_to_target_format(dt2)
            list_for_create_diaposon = [date1_str, date2_str]
